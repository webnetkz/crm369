...
test('chat ui renders header trigger, sidebar dock, and sheet targeting hooks', function () {
    ...
    expect($header)->toContain('ChatCenterSheet')
        ...
        ->and($dock)->toContain('t.value.chat.unread')
        ->and($dock)->toContain(':aria-label="entryAriaLabel(entry)"')
        ->and($dock)->toContain('aria-hidden="true"')
        ->and($dock)->toContain('@pointerenter="refreshDockData"')
        ->and($dock)->toContain('class="relative block overflow-visible"')
        ->and($dock)->toContain('absolute -top-2 -right-2 z-30 inline-flex')
        ->and($dock)->not->toContain('{{ entry.title }}')
        ->and($dock)->not->toContain('{{ t.chat.unread }}: {{ entry.unreadCount }}')
        ...
        ->and($dock)->toContain('ring-2 ring-background')

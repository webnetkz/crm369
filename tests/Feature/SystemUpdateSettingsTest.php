...
test('system updater uses an independent allowlisted root bridge with backups and migrations', function () {
    ...
    expect($updater)
        ...
        ->toContain('--working-dir="$laravel_candidate_path"')
        ->toContain('crm369-laravel-candidate.')
        ->toContain('cleanup_laravel_candidate')
        ->toContain("laravel_dependencies_switched='true'")
    ...
    expect(strpos($applicationUpdate, 'maintenance_begin'))
        ...
        ->and(strpos($laravelUpdate, 'composer_command update laravel/framework'))
        ->toBeLessThan(strpos($laravelUpdate, 'maintenance_begin'))
        ->and(strpos($laravelUpdate, 'maintenance_begin'))
        ->toBeLessThan(strpos($laravelUpdate, 'database_backup'))
        ->and(strpos($laravelUpdate, 'database_backup'))
        ->toBeLessThan(strpos($laravelUpdate, 'mv "${APP_PATH}/vendor" "${laravel_backup_path}/vendor"'))

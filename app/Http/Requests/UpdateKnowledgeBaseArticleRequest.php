...
class UpdateKnowledgeBaseArticleRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $articleId = $article instanceof KnowledgeBaseArticle ? $article->id : null;
        ...
        return [
            'parent_id' => [
                ...
                function (string $attribute, mixed $value, \Closure $fail) use ($articleId): void {
                    if ($articleId !== null && is_numeric($value) && (int) $value === $articleId) {
                        $fail(__('ui.knowledge.validation_parent_self'));
                    }
                },
            ],
            ...
            'slug' => [
                ...
                Rule::unique('knowledge_base_articles', 'slug')
                    ->where(fn ($query) => $knowledgeBaseId ? $query->where('knowledge_base_id', $knowledgeBaseId) : $query)
                    ->ignore($articleId),
            ],

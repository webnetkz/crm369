...
class UpdateEdoDocumentRequest extends FormRequest
{
    /**
    ...
    private function existingDocument(): ?EdoDocument
    {
        $edoDocument = $this->route('edoDocument');

        return $edoDocument instanceof EdoDocument ? $edoDocument : null;
    }
    ...
    private function validateSourcePayload(Validator $validator): void
    {
        ...
        if ($this->documentSource() === EdoDocument::SOURCE_UPLOAD) {
            if ($this->uploadedDocument() === null
                && ! ($this->existingDocument()?->document_source === EdoDocument::SOURCE_UPLOAD
                    && $this->existingDocument()?->hasDocumentFile())
            ) {
        ...
        if (! $selectedFileEntry
            && ! ($this->existingDocument()?->document_source === EdoDocument::SOURCE_FILE_ENTRY
                && $this->existingDocument()?->hasDocumentFile())
        ) {

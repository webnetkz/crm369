...
import { ArrowLeft, Boxes, Building2, Package, QrCode, Rows3 } from '@lucide/vue';
...
const selectedColumnId = ref<number | null>(props.map.rows[0]?.columns[0]?.id ?? null);
const selectedFloorId = ref<number | null>(props.map.rows[0]?.columns[0]?.floors[0]?.id ?? null);
...
const selectedColumn = computed<MapColumn | null>(() => {
    for (const row of props.map.rows) {
        const match = row.columns.find((column) => column.id === selectedColumnId.value);
...
const selectedRow = computed<MapRow | null>(() => {
    return props.map.rows.find((row) => row.columns.some((column) => column.id === selectedColumnId.value)) ?? null;
});

const selectedFloor = computed<MapFloor | null>(() => {
    return selectedColumn.value?.floors.find((floor) => floor.id === selectedFloorId.value) ?? null;
});
...
const updateInventoryPerPage = (value: number): void => {
    router.get(showWarehouse.url(props.warehouse.id), {
...
<template>
    ...
    <div class="space-y-8">
            ...
            >
                <div class="space-y-4">
                    ...
                    <div class="rounded-2xl border border-border bg-background/80 px-4 py-3 text-sm">
                        <div class="text-muted-foreground">{{ t.warehouses.warehouse_qr }}</div>
        ...
        <section class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
                    ...
                    >
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div class="flex flex-wrap gap-2 text-xs">
                                ...
                                <div class="rounded-full border border-border bg-card px-3 py-1.5">
                                    {{ t.warehouses.metric_items }}: {{ formatNumber(row.item_count) }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                            <button
                                ...
                                :class="column.id === selectedColumnId
                            ...
                            >
                                ...
                                <div class="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                                    <span>{{ t.warehouses.metric_floors }}: {{ formatNumber(column.floor_count) }}</span>
                                    <span>{{ t.warehouses.metric_places }}: {{ formatNumber(column.place_count) }}</span>
                                    <span>{{ t.warehouses.metric_items }}: {{ formatNumber(column.item_count) }}</span>
                                </div>
                            </button>
                        </div>
                    </article>
            ...
            <aside class="rounded-3xl border border-border bg-card p-6">
                <Heading
                    ...
                    :description="selectedRow ? `${selectedRow.name}` : t.warehouses.map_column_panel_description"
                />

                <div v-if="selectedColumn" class="mt-6 space-y-4">
                    <div class="rounded-2xl border border-border bg-background/70 p-4">
                        <div class="font-semibold">{{ selectedColumn.name }}</div>
                        <div class="mt-3 flex flex-wrap gap-2 text-xs">
                            <div class="rounded-full border border-border bg-card px-3 py-1.5">
                                {{ t.warehouses.metric_floors }}: {{ formatNumber(selectedColumn.floor_count) }}
                            </div>
                            <div class="rounded-full border border-border bg-card px-3 py-1.5">
                                {{ t.warehouses.metric_places }}: {{ formatNumber(selectedColumn.place_count) }}
                            </div>
                            <div class="rounded-full border border-border bg-card px-3 py-1.5">
                                {{ t.warehouses.metric_items }}: {{ formatNumber(selectedColumn.item_count) }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <button
                            ...
                            :class="floor.id === selectedFloorId
                                ? 'border-primary bg-primary/10'
                        ...
                        >
                            ...
                            <div class="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                                <span>{{ t.warehouses.metric_places }}: {{ formatNumber(floor.place_count) }}</span>
                                <span>{{ t.warehouses.metric_items }}: {{ formatNumber(floor.item_count) }}</span>
                            </div>
                        </button>
                    </div>

                    <div v-if="selectedFloor" class="space-y-3">
                        <div class="text-sm font-medium">{{ t.warehouses.map_floor_places }}</div>

                        <div
                            v-if="selectedFloor.places.length === 0"
                        ...
                        <article
                            v-for="place in selectedFloor.places"
                            :key="place.id"
                        ...
                        >
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <div class="text-xs text-muted-foreground">
                                    {{ t.warehouses.metric_items }}: {{ formatNumber(place.item_count) }}
                                </div>
                            </div>

                            <div v-if="place.items.length > 0" class="mt-3 space-y-2">
                                <div
                                ...
                                >
                                    ...
                                    <div class="mt-1 flex flex-wrap gap-2 text-xs text-muted-foreground">
                                        <span v-if="item.sku">{{ t.warehouses.sku }}: {{ item.sku }}</span>
                                        <span>{{ t.warehouses.quantity }}: {{ item.quantity }}</span>
                                    </div>
                                </div>
                            </div>
        ...
        <section class="rounded-3xl border border-border bg-card p-6">
            ...
            >
                <div class="text-lg font-semibold">{{ t.warehouses.inventory_empty }}</div>
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <div class="mt-1 text-sm text-muted-foreground">
                                    ...
                                    <span class="text-foreground">{{ item.location.path }}</span>
                                </div>
                            </div>

                            <div class="rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                                {{ t.warehouses.quantity }}: {{ item.quantity }}
                            </div>
                        </div>

                        <div class="mt-4 grid gap-3 sm:grid-cols-2">
                            <div class="rounded-xl border border-border bg-card px-3 py-3 text-sm">
                                <div class="text-muted-foreground">{{ t.warehouses.sku }}</div>
                                <div class="mt-1 font-medium">{{ item.sku ?? '—' }}</div>
                            </div>
                            <div class="rounded-xl border border-border bg-card px-3 py-3 text-sm">
                                <div class="text-muted-foreground">{{ t.warehouses.scan_matched_qr }}</div>
                                <div class="mt-1 font-mono font-medium">{{ item.qr_code }}</div>
                            </div>
                        </div>
                    </article>
                </div>
